...
The `discord_setup` module provides a decorator-based system for simplifying Discord bot development. It allows you to write command handlers, event listeners, and UI components in a clean, modular way without manually creating cogs or handling discord.py's lower-level APIs.

## Overview

This module is built around Python decorators that mark functions as Discord commands, listeners, buttons, and selects. These decorators attach metadata to functions, which is then read by the module's utilities to automatically construct Discord cogs and UI components.
